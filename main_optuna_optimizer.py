...
os.makedirs("optuna_results_optimizer_population", exist_ok=True)
...
def objective(trial):
    ...
    config['output_csv_folder'] = "Results" + "_" + config['base_experiment_name'] + "_Optimizer_Population"
    config['experiment_name'] = f"OPT_OPTIMIZER_POPULATION_Trial_{trial.number}_W{config['warmup_epochs']}_G{config['gens_per_epoch']}"
    config['seed'] = 0 
    ...
    master_model.current_optimizer = None # To sure that the optimizer will be create at the first training epoch and to be maintained
    ...
    for epoch in range(config['start_epochs'], config['stop_epochs'] + 1):
        ...
        if epoch > config['warmup_epochs']:
            best_ind, population = EA.ea_step_optimizer(config, epoch, population, master_model)
        ...
        if val_acc > best_val_acc_in_trial:
            ...
            model_path = f"optuna_results_optimizer_population/model_trial_{trial.number}.pth"
...
if __name__ == "__main__":
    ...
    storage_db = "sqlite:///hpo_medmnist_optimizer_population_study.db"
    
    study = optuna.create_study(
        study_name="MedMNIST_EA_Optimization_population_Optimizer",
        direction='maximize',
        storage=storage_db,
        load_if_exists=True,
    ...
    df_results.to_csv("optuna_hpo_final_optimizer_population_summary.csv", index=False)
