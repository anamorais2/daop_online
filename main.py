...
def completed_run(config):
    ...
    config['start_epochs'] = max_epoch + 1
...
def load_and_test(model_path, config, dataset_vars):
    print(f"\n[TEST MODE] Loading weights from: {model_path}")
    
    master_model = config['model'](num_classes=config['num_classes'])
    
    try:
        state_dict = torch.load(model_path, map_location=config['device'])
        master_model.model.load_state_dict(state_dict)
        master_model.model.to(config['device'])
        print("Weights loaded successfully.")
    except Exception as e:
        print(f"Error loading weights: {e}")
        return

    print(f"\n==== Final Test Set Evaluation - Load Mode ====")
    config['evaluate_on_test'] = True
    config["epochs"] = config['final_eval_epochs']
    
    test_acc, history = config['individual_evaluation_func'](
        [], 
        config, 
...
ONLY_TEST = False
MODEL_PATH_TO_LOAD = "optuna_results/model_trial_30.pth"
...
if __name__ == "__main__":
    ...
    for seed in config['seeds']:
        ...
        if ONLY_TEST:
            load_and_test(MODEL_PATH_TO_LOAD, config, dataset_vars)
            continue 
        ...
        for epoch in range(config['start_epochs'], config['stop_epochs'] + 1):
            ...
            if epoch > config['warmup_epochs']:
                ...
                current_DA = []  # No augmentations during warmup
